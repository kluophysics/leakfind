C*==dirbssig.f    processed by SPAG 6.70Rc at 15:35 on 19 Dec 2016
      SUBROUTINE DIRBSSIG(GETIRRSOL,C,E,L,MJ,KAP1,KAP2,P,CG1,CG2,CG4,
     &                    CG5,CG8,V,B,Z,R,DRDI,DOVR,IRTOP,IRCUT,NPAN,PR,
     &                    QR,PI,QI,DP,DQ,NRMAX,GF_CONV_RH,WDMFT)
C   ********************************************************************
C   *                                                                  *
...
Cppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp
      DO IPAN = 1,NPAN
C
         CALL DIRBSRADSIG(X,Y,DY,DRDI,B,V,R,NRMAX,NCFMAX,WDMFT)
C
...
C
            CALL DIRBSSTPSIG(Y,DY,NY,X,HBS,B,V,R,DRDI,YM,YN,YERR,YSAV,
     &                       YSEQ,DYSAV,NRMAX,NCFMAX,WDMFT)
C
            NY = 0
            DO J = 1,NSOL
               DO I = 1,NSOL
...
C
      END
C*==dirbsstpsig.f    processed by SPAG 6.70Rc at 15:35 on 19 Dec 2016
      SUBROUTINE DIRBSSTPSIG(Y,DY,NY,X,HBS,B,V,R,DRDI,YM,YN,YERR,YSAV,
     &                       YSEQ,DYSAV,NRMAX,NCFMAX,WDMFT)
C   ********************************************************************
C   *                                                                  *
...
C
      ...
      COMPLEX*16 B(NRMAX),DY(NY),DYSAV(NCFMAX),V(NRMAX),WDMFT(2,2),
     &           Y(NY),YERR(NCFMAX),YM(NCFMAX),YN(NCFMAX),YSAV(NCFMAX),
     &           YSEQ(NCFMAX)
...
C
         ...
         XMID = X + HMID
C
         CALL DIRBSRADSIG(XMID,YN,YSEQ,DRDI,B,V,R,NRMAX,NCFMAX,WDMFT)
C
         H2MID = 2.D0*HMID
         DO ISTEP = 2,NSEQ(I)
            DO IY = 1,NY
...
C
      END
C*==dirbsradsig.f    processed by SPAG 6.70Rc at 15:35 on 19 Dec 2016
      SUBROUTINE DIRBSRADSIG(XBS,Y,DYDX,DRDI,B,V,R,NRMAX,NCFMAX,WDMFT)
C   ********************************************************************
C   *                                                                  *
...
C
      ...
      REAL*8 XBS
      COMPLEX*16 B(NRMAX),DYDX(NCFMAX),V(NRMAX),WDMFT(2,2),Y(NCFMAX)
      REAL*8 DRDI(NRMAX),R(NRMAX)
C
C Local variables
C
      COMPLEX*16 BBS,BPP,BPP_CGO,BQQ,EMVPP,EMVPP_BPP_CGD(2),EMVQQ,
     &           EMVQQ_BQQ_CGMD(2),VBS,WDMFTA(2,2)
...
C
     ...
     &                  + BPP_CGO*Y(M+K) + WDMFTA(I,I)*Y(M+1)
     &                  + WDMFTA(I,3-I)*Y(M+K)
            M = M + 2
         END DO
      END DO
