C*==me_spin_alf_alf.f    processed by SPAG 6.70Rc at 15:37 on 19 Dec 2016
      SUBROUTINE ME_SPIN_ALF_ALF(IFILA,ERYDA,IFILB,ERYDB,IT,MZAZB,MZBZA,
...
C
      ...
      PARAMETER (ROUTINE='ME_SPIN_ALF_ALF')
      INTEGER NPOL
      PARAMETER (NPOL=3)
C
C Dummy arguments
...
C
     ...
     &           PAB,PBA,SJAJB,SJAZB,SZAJB,SZAZB,ZFLA(:,:,:),ZFLB(:,:,:)
     &           ,ZFLBP(:,:,:),ZFRA(:,:,:),ZFRAP(:,:,:),ZFRB(:,:,:),
     &           ZGLA(:,:,:),ZGLB(:,:,:),ZGLBP(:,:,:),ZGRA(:,:,:),
     &           ZGRAP(:,:,:),ZGRB(:,:,:)
...
C
      ...
      END DO
C
      CALL ME_SPIN_CURR_ALF_B4A1(IM,IMC,PBA,ZGLB,ZFLB,ZGRA,ZFRA,ZGLBP,
     &                           ZFLBP,ZGRAP,ZFRAP,MZBZA,MZBZANAB)
C
CC---------------------------------------- for nabla related surface term
CC-------------------------------------------------------------------  ZZ
C      CALL ME_SPIN_CURR_ALF_BXAY_NAB(IM,ZGLB,ZFLB,ZGRA,ZFRA,ZGLBP,ZFLBP,
...
C                                                      energy B -- LAMB4
                     DO LAMB4 = 1,NKM
C
                        CALL ME_SPIN_CURR_ALF_BXAXW(IM,IMC,PBA,LAMB4,
     &                     ZGLB,ZFLB,ZGLB,ZFLB,LAMA1,ZGRA,ZFRA,JGRA,
     &                     JFRA,ZGLBP,ZFLBP,ZGLBP,ZFLBP,ZGRAP,ZFRAP,
     &                     JGRAP,JFRAP,IZAZB,SZAZB,IJAZB,SJAZB,MIRR_2,
     &                     LAMB4,LAMB3,JPOL,MZBZANAB)
C
                     END DO
...
C                                                      energy A -- LAMA1
                     DO LAMA1 = 1,NKM
C
                        CALL ME_SPIN_CURR_ALF_BXAXW(IM,IMC,PBA,LAMB3,
     &                     ZGLB,ZFLB,JGLB,JFLB,LAMA1,ZGRA,ZFRA,ZGRA,
     &                     ZFRA,ZGLBP,ZFLBP,JGLBP,JFLBP,ZGRAP,ZFRAP,
     &                     ZGRAP,ZFRAP,IZAZB,SZAZB,IZAJB,SZAJB,MIRR_3,
     &                     LAMA2,LAMA1,JPOL,MZBZANAB)
C
                     END DO
...
C
                     LAMA1 = LAMA2
C
                     CALL ME_SPIN_CURR_ALF_BXAXW(IM,IMC,PBA,LAMB3,ZGLB,
     &                  ZFLB,JGLB,JFLB,LAMA1,ZGRA,ZFRA,JGRA,JFRA,ZGLBP,
     &                  ZFLBP,JGLBP,JFLBP,ZGRAP,ZFRAP,JGRAP,JFRAP,IZAZB,
     &                  SZAZB,IJAJB,SJAJB,MIRR_4,LAMA1,LAMB3,JPOL,
     &                  MZBZANAB)
C
C***********************************************************************
C
...
C
      END
C*==me_spin_curr_alf_b4a1.f    processed by SPAG 6.70Rc at 15:37 on 19 Dec 2016
      SUBROUTINE ME_SPIN_CURR_ALF_B4A1(IM,IMC,PBA,ZGLB,ZFLB,ZGRA,ZFRA,
     &                                 ZGLBP,ZFLBP,ZGRAP,ZFRAP,MZBZA,
     &                                 MZBZANAB)
C   ********************************************************************
C   *                                                                  *
C   *  evaluate the matrix elements of the type                        *
...
C
      CHARACTER*40 ROUTINE
      PARAMETER (ROUTINE='ME_SPIN_CURR_ALF_B4A1')
      INTEGER NPOL
      PARAMETER (NPOL=3)
C
C Dummy arguments
...
C Local variables
C
      REAL*8 AF1_B4A1,AF2_B4A1,AF3_A1B4,AF4_A1B4,AG1_B4A1,AG2_B4A1,
     &       AG3_A1B4,AG4_A1B4
      COMPLEX*16 CADD,CINT1(:),CINT2(:),CINT3(:),CINT4(:),FAG,FZBZA(:),
     &           GAF,IMCA1,IMCA2,ME_PM0(3,3),ME_P_XYZ(3,3),PA,PRE_NABLA,
     &           R2_ZFLB,R2_ZFRA,R2_ZGLB,R2_ZGRA,RMEF1(2,2),RMEF2(2,2),
     &           RMEF3(2,2),RMEF4(2,2),RMEG1(2,2),RMEG2(2,2),RMEG3(2,2),
     &           RMEG4(2,2),S2BAR,TERM12,TERM34,V2BAR
      LOGICAL CNON0
      INTEGER IA1,IA_ERR,IB4,IFLAG1,IKMA1,IKMB4,INDXIPOLR(3),IPOL,IPOLR,
     &        IPOL_SPIN,IPOL_SPINR,IR,IRTOP,LA1,LAMA1,LAMB4,LB4,LBA1,
     &        LBB4
C
C*** End of declarations rewritten by SPAG
C
      DATA INDXIPOLR/2,1,3/
C
...
C
CDK equivalent to above ----------------------------------------- BEGIN
                        IF ( 1.EQ.0 ) THEN
                           IPOLR = INDXIPOLR(IPOL)
                           IPOL_SPINR = INDXIPOLR(IPOL_SPIN)
C
                           AG3_A1B4 = AG1_SPIN_CURR_NAB(IKMA1,IKMB4,
     &                                IPOLR,IPOL_SPINR)
                           AG4_A1B4 = AG2_SPIN_CURR_NAB(IKMA1,IKMB4,
     &                                IPOLR,IPOL_SPINR)
                           AF3_A1B4 = AF1_SPIN_CURR_NAB(IKMA1,IKMB4,
     &                                IPOLR,IPOL_SPINR)
                           AF4_A1B4 = AF2_SPIN_CURR_NAB(IKMA1,IKMB4,
     &                                IPOLR,IPOL_SPINR)
C
C                           AF3_A1B4 = 0d0
C                           AF4_A1B4 = 0d0
C
     ...
     &                              RMEF4(IA1,IB4)*AF4_A1B4
C
                           IF ( IPOL_SPIN.EQ.3 ) TERM34 = -TERM34
C
                        END IF
CDK equivalent to above ----------------------------------------- END
C
                        CADD = TERM12 + TERM34
C
...
C
      END
C*==me_spin_curr_alf_bxaxw.f    processed by SPAG 6.70Rc at 15:37 on 19 Dec 2016
      SUBROUTINE ME_SPIN_CURR_ALF_BXAXW(IM,IMC,PBA,LAMB,ZGLB,ZFLB,JGLB,
     &                                  JFLB,LAMA,ZGRA,ZFRA,JGRA,JFRA,
     &                                  ZGLBP,ZFLBP,JGLBP,JFLBP,ZGRAP,
     &                                  ZFRAP,JGRAP,JFRAP,IZAZB,SZAZB,
     &                                  IJAJB,SJAJB,MIRR,I,J,JPOL,
     &                                  MZBZANAB)
C   ********************************************************************
C   *                                                                  *
C   *  evaluate the matrix elements of the type                        *
...
C
      CHARACTER*40 ROUTINE
      PARAMETER (ROUTINE='ME_SPIN_CURR_ALF_BXAXW')
      INTEGER NPOL
      PARAMETER (NPOL=3)
C
C Dummy arguments
...
C Local variables
C
      COMPLEX*16 A1JFRA,A1ZFRA,A2JGRA,A2ZGRA,CADD,CINT1(:),CINT2(:),
     &           CINT3(:),CINT4(:),FS(:),FV(:),IMCA1,IMCA2,ME_PM0(3,3),
     &           ME_P_XYZ(3,3),PA,PRE_NABLA,R2_IJAJB,R2_IZAZB,R2_JFLB,
     &           R2_JFRA,R2_JGLB,R2_JGRA,R2_ZFLB,R2_ZFRA,R2_ZGLB,
     &           R2_ZGRA,RMEF1(2,2),RMEF2(2,2),RMEF3(2,2),RMEF4(2,2),
     &           RMEG1(2,2),RMEG2(2,2),RMEG3(2,2),RMEG4(2,2),S2REG,
     &           SCNTR,SCNTRNAB(3),TERM12,TERM34,VCNTR,VIRR,WIRR,WREG
      REAL*8 AF1_B4A1,AF2_B4A1,AF3_A1B4,AF4_A1B4,AG1_B4A1,AG2_B4A1,
     &       AG3_A1B4,AG4_A1B4
      LOGICAL CNON0
...
C
                        WREG = WREG*R2DRDI(IR,IM)
                        WIRR = WIRR*R2DRDI(IR,IM)
C
