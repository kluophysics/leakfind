C*==signlc.f    processed by SPAG 6.70Rc at 15:39 on 19 Dec 2016
      SUBROUTINE SIGNLC(IPRINT,WRMAT,NDIMCLUSQ,IND0QCLU,RQNLCPA,KNNLCPA,
...
C   ********************************************************************
     ...
     &    TAUQ,TAUT,TSST
      USE MOD_SITES,ONLY:NQ,NQMAX,ITOQ,NOQ,IQAT,IQBOT_CHI,IQTOP_CHI
...
C
      ...
      COMPLEX*16 CHIHATZ(:,:,:),DERYD,DMAMC(:,:),DMATTA(:,:,:),
     &           DMATTB(:,:,:),DTILTA(:,:,:),DTILTB(:,:,:),ERYD,ERYD9,
     &           ERYDA,ERYDB,MAQAB(:,:,:,:),MAQBA(:,:,:,:),
     &           MAQQAB(:,:,:,:,:),MAQQBA(:,:,:,:,:),MBQAB(:,:,:,:),
     &           MBQBA(:,:,:,:),MBQQAB(:,:,:,:,:),MBQQBA(:,:,:,:,:),
     &           MCQAB(:,:,:,:),MCQBA(:,:,:,:),MCQQAB(:,:,:,:,:),
     &           MCQQBA(:,:,:,:,:),MDQAB(:,:,:,:),MDQBA(:,:,:,:),
     ...
     &           OMEGAHATB(:,:),P,PA,PB,SIG0Q(3,3,NQMAX),
     &           SIG1_NVQ(3,3,NQMAX,NQMAX),SIG1_VCQ(3,3,NQMAX,NQMAX),
     &           TAUHAT(:,:),TAUIMP(:,:),TAUQA(:,:,:),TAUQB(:,:,:),
     &           TAUQZ(NKMMAX,NKMMAX,NQMAX,2),TAUTA(:,:,:),TAUTB(:,:,:),
     &           W1HAT(:,:)
      REAL*8 CPACHNG,CPACHNGMAX,CPAERR,RHOAU_NV,RHOAU_VC,RHO_NV(3,3),
     &       RHO_VC(3,3),SIGMAAU_NV,SIGMAAU_VC,SIGMA_NV(3,3),
     &       SIGMA_VC(3,3),TIME,TIME0,WGIT(:)
...
C
      ...
      IF ( IA_ERR.NE.0 ) CALL STOP_MESSAGE(ROUTINE,'allocate W1HAT')
C
...
C
      ...
      IF ( IA_ERR.NE.0 ) CALL STOP_MESSAGE(ROUTINE,'allocate MAQAB')
C
...
C
     ...
     &              CALL STOP_MESSAGE(ROUTINE,'>>> FILE 9')
               TAUQ(:,:,IQ) = TAUQA(:,:,IQ)
            END DO
C
...
C ======================================================================
C
            CALL SIGNLCME(ERYDA,ERYDB,IFILA,IFILB,IQCPA,NQNLCPA,NDIMCLU,
...
C
            ELSE
C
               CALL SIG0(INTEG,MTAB,MTBA,MIRRTAB,TAUTA,TAUTB,SIG0Q,
     &                   SIG_MODE)
C
            END IF
C
            CALL SIGNLC1(CHIHATZ,MAQQAB,MBQQAB,MCQQAB,MDQQAB,SIG1_NVQ,
...
C
     ...
     &                      SIG1_VCQ,'V',IPRINT,NQNLCPA,NKM,NKMMAX,
     &                      NQMAX,NZ12MAX,NDIMCHI)
C
            ELSE
C
               CALL ZCOPY(3*3*NQMAX*NQMAX,SIG1_NVQ,1,SIG1_VCQ,1)
C
            END IF
C
            CALL SIGNLCSUM(SIG1_NVQ,SIG1_VCQ,SIG0Q,SIGMAAU_NV,
